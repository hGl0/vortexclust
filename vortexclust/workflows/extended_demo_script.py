...
# helper class (file-like object that logs line-by-line)
class _StreamToLogger(io.TextIOBase):
    ...
    def write(self, s):
        # buffer until newline, then log clean lines
        self._buf += s
        while "\n" in self._buf:
...
def main():
    ...
    out = _StreamToLogger(logger, logging.INFO)
    err = _StreamToLogger(logger, logging.ERROR)

    with redirect_stdout(out), redirect_stderr(err):
        # === Paths from config ===
        demo_d = Path(config.DATA_DIR + "/demo_d.csv")
        if not demo_d.exists():
            raise FileNotFoundError(f"{demo_d} does not found. Make sure your working directory is correct. "
                          f"The demo script sets it automatically to the root directory of the script."
                          f"Otherwise edit your local config.py")
        demo_msw = Path(config.DATA_DIR +"/demo_msw.csv")
        output_dir = Path(config.OUTPUT_DIR +"/demo_out")
        ...
        plot_hist_per_class(
            ...
            savefig=str(output_dir / "demo_hist_per_class_")
        )
